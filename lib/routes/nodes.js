  ...
  .post(function(req, res) {
    function saveCommand(collection) {
      collection.update(
        ...
        {$pull: {'commands': {'type': command.type}}},
        {safe: true}, function(err, result) {
          if (err) {
            return next(err);
          }
          console.log('command pull: ' + result );
          collection.update(
            {'id': req.params.node_id},
            ...
            {safe: true}, function(err, result) {
              ...
              console.log('command push: ' + result );
...
router.route('/public/nodes/sync')
  ...
  .post(function(req, res, next) {
    ...
    async.waterfall([
      ...
      function(node, callback) {
        node.commands.forEach(function(c) {
          c.status = 'running';
        });
        callback(null, node.commands);
      },
