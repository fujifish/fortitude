...
router.route('/nodes/:id')
  ...
  .put(function(req, res) {
    ...
    db.collection('nodes', function(err, collection) {
      if(err) return next(err);
      collection.update({'_id':new BSON.ObjectID(id)}, node, {safe:true}, function(err, result) {
        if(err) return next(err);
  ...
  .post(function(req, res, next) {
    ...
    async.waterfall([
      ...
      function(node, callback) {
        nodeCollection.updateOne({'id':input.id}, {$set: {id: input.id, name: input.name, currentState: input.payload.modules, info: input.payload.info, lastSync: new Date() }}, {safe:true, upsert: true}, callback);
