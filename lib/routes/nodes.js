...
router.route('/nodes/:node_id/commands')
  .post(function(req,res, next){
    function saveCommand(collection){
        collection.update(
          {'id': req.params.node_id},
          {$pull: {'commands': { 'type': command.type }}, $push: {"commands": command}},
      ...
      switch(command.type){
        case 'state.apply':
          collection.findOne({'id': req.params.node_id}, function(err, node) {
            if(err) return next(err);
            command.state = node.plannedState;
            saveCommand(collection);
          });
          break;
        default:
          saveCommand(collection);
  ...
  .post(function(req, res, next) {
    ...
    async.waterfall([
      ...
      function(node, callback) {
        nodeCollection.updateOne({'id':input.id}, {$set: {id: input.id, name: input.name, "state.current" :  input.payload.modules , info: input.payload.info, lastSync: new Date() }}, {safe:true, upsert: true}, callback);
    ...
    db.collection('nodes', {strict:true}, function(err, collection) {
      if (err) {
        ...
        db.createCollection('nodes', function(err, result) {
          if (err) console.log("failed to create collection nodes, err = " + err);
        });
      }
    });
  }
