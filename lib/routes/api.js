  ...
  .post(function(req, res) {
    ...
    db.collection('modules', function(err, collection) {
      collection.findOne({name: module.name, version: module.version}, function(err, item) {
        ...
        if (item) {
          return next('Failed to add module: already exists');
        }
...
router.route('/modules/:name/:version')
  ...
  .delete(function(req, res) {
    var module = req.body;
    console.log('Removing module: ' + JSON.stringify(module));
    db.collection('modules', function(err, collection) {
      collection.deleteOne({name: module.name, version: module.version}, function(err, result) {
        if (err) {
          return next(err);
        }
        console.log('Success: ' + JSON.stringify(result[0]));
