  ...
  $(function() {
    ...
    function updateModulesTable(data) {
      ...
      table.bootstrapTable({
        ...
        data: data.map(function(obj, index) {
          return {name: obj.name, version: obj.version, actions: modulesTableActions(index), obj: obj};
        }),
    ...
    function updateNodeModulesTable(element, modules, editable) {
      modules = modules || [];
      element.bootstrapTable('destroy');
      element.bootstrapTable({
        classes: 'table table-no-bordered cursor-hand',
        ...
        onClickRow: function(row, $element) {
          ...
          if (editable) {
            $('#edit-node-module-title').html('Edit ' + row.name + '@' + row.version);

            //$('#edit-node-module-configuration').val(JSON.stringify(row.obj.state.configure.data, null, 2));
            ...
            if (row.obj.state && row.obj.state.start && row.obj.state.start.data) {
              if (row.obj.state.start.data.started) {
                $('#edit-node-module-started').prop("checked", row.obj.state.start.data.started);
              } else {
    ...
    $('#edit-node-module-save').click(function() {
      $('#edit-node-module-dialog').modal('hide');
      var nodeDetails = $("#node-details");
      var nodeRow = nodeDetails.data("row");
      var moduleRow = nodeDetails.data("moduleRow");
