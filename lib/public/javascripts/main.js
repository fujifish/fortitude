(function() {
  ...
  $(function() {
    ...
    function updateCommandsTable(element, commands) {
      ...
      element.bootstrapTable({
        ...
        data: commands.map(function(obj) {
          return {name: obj.name, state: obj.state, creationDate: obj.creationDate, obj: obj};
        }),
        cardView: false,
    ...
    function displayNodeDetails(row) {
      ...
      updateCommandsTable($('#commands-table'), row.obj.commands);
    }
    ...
    $('#applyState').click(function(){
      ...
      HB.NODES.applyState(row.id, function(err, node) {
        /*if (!err) {
          refreshNodeRow(node);
        }*/
      });
    ...
    window.moduleform = $('#moduleform').jsonForm({
      schema: {
        name: {
          type: 'string',
          title: 'Name',
