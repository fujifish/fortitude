...
// Twitter bootstrap-friendly HTML boilerplate for standard inputs
  ...
  var inputFieldTemplate = function (type) {
    return {
      ...
      'class="form-control col-sm-12 <%= fieldHtmlClass || "" %>" ' +
      'name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.readOnly ? " readonly=\'readonly\'" : "") %>' +
      '<%= (node.schemaElement && node.schemaElement.maxLength ? " maxlength=\'" + node.schemaElement.maxLength + "\'" : "") %>' +
      '<%= (node.schemaElement && node.schemaElement.required && (node.schemaElement.type !== "boolean") ? " required=\'required\'" : "") %>' +
      'placeholder="<%= escape(node.placeholder || node.title) %>" ' +
      ' />',
      'fieldtemplate': true,
      'inputfield': true
    }
  ...
  jsonform.elementTypes = {
    ...
    'submit':{
      'template':'<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> class="btn btn-primary pull-right <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%>/>'
    },
    'button':{
